...
// ── 문체 린트 (결정론적 가드) ─────────────────────────────────
// 모델이 직전 화를 보고 이탤릭·"박자" 손버릇을 눈덩이처럼 따라하는 걸 코드로 차단.
// 프롬프트 지시문만으론 못 막혀(STYLE에 있어도 48개까지 갔던 전례) → 세서 넘으면 재생성.
const ITALIC_MAX = 6;     // *...* 강조 화당 최대치(본문 길이 무관 절대 상한)
const BAKJA_MAX = 2;      // "박자" 표현 화당 최대치
function styleLint(body) {
  const italics = (body.match(/(?<!\*)\*(?!\*)[^*\n]+?\*(?!\*)/g) || []).length; // *강조*만(**볼드** 제외)
  const bakja = (body.match(/박자/g) || []).length;
  const issues = [];
  if (italics > ITALIC_MAX) issues.push(`- [문체/이탤릭] *강조*를 ${italics}개 썼다(상한 ${ITALIC_MAX}). 강조는 화당 ${ITALIC_MAX}개 이하. 정말 결정적인 단어 1~2개만 남기고 전부 평문으로. 평범한 단어(좌표·반사·식는다 등)는 강조 금지.`);
  if (bakja > BAKJA_MAX) issues.push(`- [문체/반복] "박자"를 ${bakja}번 썼다(상한 ${BAKJA_MAX}). 손버릇이다. "한 박자 멈췄다/늦었다" 류를 다른 표현(멈칫·반 걸음·한 호흡·잠깐)으로 분산하거나 삭제.`);
