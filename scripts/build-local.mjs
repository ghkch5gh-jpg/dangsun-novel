...
// ── 정통 문법 ─────────────────────────────────────────────────
const STYLE = `# 한국 양산형 웹소설 문법 — 대중성·재미 최우선 (반드시 준수)
- **첫 3줄 후킹**: 풍경·분위기·설명으로 시작 절대 금지. 첫 문장부터 사건/대사/위기/사이다 예고로 멱살 잡고 끌어당겨라.
- **사이다 ≥ 1 (직접적·시각적)**: 매 화 반드시, 주인공이 적을 *눈앞에서 납작하게* 누르거나 한 방 먹이는 통쾌함. 머리만 굴리는 것 말고 보이는 쾌감으로(상대의 표정·말문 막힘·반전 역전). 고구마 금지.
- **절단신공**: 마지막 1~2문장은 강한 훅/반전/위기로 끊어 다음 화를 못 참게.
- **문체(대중성 핵심)**: 문장 짧고 단순하게. **대사 비중 아주 높게(가능하면 본문 절반 이상 대사)**. 한 문단 2~4문장 이내. 묘사·함의·미문·만연체 최소 — 폰으로 빠르게 스킵해도 즉각 이해되게. 어려운 표현 금지.
- **속도**: 한 화 안에 사건이 확확 진행. 늘어지는 내면묘사·분위기 잡기 금지. 한 화 = 명확한 사건 하나 + 한 방.
- **차별점 유지(THE HOOK)**: 아래 'premise(한 끗)'를 매 화 살려라. 평범한 "회귀자=미래 다 앎 무쌍" 클리셰로 회귀 금지.
- 분량: 본문 **1800~2600자** (짧게! 길게 늘이지 말 것). 5분 안에 읽히게.
